...
export const useUIStore = create<UIStore>((set) => ({
	...
	openPanel: (key) =>
		set((s) => ({
			panel: { ...s.panel, open: !!buildPanelContent(key), content: buildPanelContent(key) },
		})),

	closePanel: () => {
		set((s) => ({ panel: { ...s.panel, open: false } }))
		setTimeout(() => set((s) => ({ panel: { ...s.panel, content: null } })), 300)
	},
...
export const openPanel = (key: SceneObjectKey) => useUIStore.getState().openPanel(key)
...
export const dismissMobileNotice = () => useUIStore.getState().dismissMobileNotice()
