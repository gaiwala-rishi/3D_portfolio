...
export default function FallbackContent() {
	...
	return (
		<div className={`${styles.fallbackRoot}${isHidden ? ' ' + styles.webglHidden : ''}`}>
			{/* ── Header ───────────────────────────────────────── */}
				...
				<section id="skills" aria-labelledby="skills-heading">
					...
					<div className={styles.skillGrid}>
						{(['frontend', 'backend', 'tools'] as const).map((cat) => (
							<article key={cat}>
								<h3>{cat === 'tools' ? 'Tools & DevOps' : cat.charAt(0).toUpperCase() + cat.slice(1)}</h3>
								<ul>
					...
					<div className={styles.projectGrid}>
						{resume.projects.map((project) => (
							<article key={project.name} className={styles.projectCard}>
								...
								{project.url && (
									<a href={project.url} rel="noopener" className={styles.projectLink}>
										View project →
									</a>
								)}
							</article>
