...
export function buildClockMesh(scene: THREE.Scene, gltf: GLTFResult): ClockMeshResult {
	...
	function makeHand(w: number, len: number, m: THREE.Material, zOff: number): THREE.Group {
		const pivot = new THREE.Group()
		pivot.position.z = zOff
		const mesh = new THREE.Mesh(new THREE.BoxGeometry(w, len, 0.008), m)
		mesh.position.y = len / 2
		pivot.add(mesh)
		const tail = new THREE.Mesh(new THREE.BoxGeometry(w * 1.1, len * 0.22, 0.008), m)
