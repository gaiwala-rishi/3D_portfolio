...
export function createLights(scene: THREE.Scene): SceneLights {
	...
	return { ambient, dirLight, fillLeft, fillRight, deskGlow, lampLight, ceilBounce, charLight }
}
...
export function updateRoomLighting(args: UpdateLightingArgs, now: Date): void {
	...
	const { ambient, dirLight, fillLeft, fillRight, ceilBounce, charLight } = lights
	...
	ambient.color.lerpColors(new THREE.Color(0xfff0e0), new THREE.Color(0xfff5ee), dayT)
	ambient.intensity = 1.5 + officeDayT * 0.5

	dirLight.color.lerpColors(new THREE.Color(0xd0d8e8), new THREE.Color(0xfff8f0), dayT)
	...
	winLight.color.lerpColors(new THREE.Color(0x7090b0), new THREE.Color(0xd4eaf5), dayT)
