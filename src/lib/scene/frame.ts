...
export function buildFrame(scene: THREE.Scene): FrameResult {
	...
	const wfSkyMat = new THREE.MeshStandardMaterial({ color: 0x2277bb, roughness: 0.9 })
	...
	const mountainPoints: [number, number][] = [
		[-0.78, -0.42], [0.78, -0.42], [0.78, -0.1], [0.55, -0.1],
		[0.4, -0.3], [0.22, -0.06], [0.05, -0.28], [-0.18, -0.02],
		[-0.38, -0.3], [-0.55, -0.1], [-0.78, -0.1],
	]
	const shape = new THREE.Shape()
	shape.moveTo(...mountainPoints[0])
	mountainPoints.slice(1).forEach((p) => shape.lineTo(...p))
	shape.closePath()
	const wfMtnMat = new THREE.MeshStandardMaterial({ color: 0x1e3e18, roughness: 0.9 })
	...
	for (let i = 0; i < 14; i++) {
		...
		const bldMat = new THREE.MeshStandardMaterial({ color: 0x2d3050, roughness: 0.9 })
	...
	return { wallFrameGroup, wfSkyMat, wfMtnMat, wfMoon, wfSun, wfStars, wfBldMats, wfWinMats }
}

export function updateWallFrameTime(fr: FrameResult, now: Date): void {
	const { wfSkyMat, wfMtnMat, wfMoon, wfSun, wfStars, wfBldMats, wfWinMats } = fr
	const h24 = now.getHours() + now.getMinutes() / 60
	const dayT =
		h24 < 5 ? 0 : h24 < 7 ? (h24 - 5) / 2 : h24 < 18 ? 1 : h24 < 20 ? 1 - (h24 - 18) / 2 : 0
	const glowT =
		h24 >= 5 && h24 < 8
	...
	const skyColor = new THREE.Color(0x0a1530).lerp(new THREE.Color(0x4499cc), dayT)
	if (glowT > 0) skyColor.lerp(new THREE.Color(0xcc6622), glowT * 0.5)
	wfSkyMat.color.copy(skyColor)
	wfMtnMat.color.lerpColors(new THREE.Color(0x1a1a40), new THREE.Color(0x1e4018), dayT)
	...
	;(wfSun.material as THREE.MeshStandardMaterial).emissiveIntensity = 1.2 + zenithT * 0.6
	...
	wfBldMats.forEach((m) =>
		m.color.lerpColors(new THREE.Color(0x1a1d38), new THREE.Color(0x3d4468), dayT)
	)
}
